...
tf.compat.v1.flags.DEFINE_bool("augmentation", False, "whether use data augmentation")
tf.compat.v1.flags.DEFINE_integer("num_models", 2, "The num of models in the ensemble")
tf.compat.v1.flags.DEFINE_integer("epoch", 1, "epoch of the checkpoint to load")

tf.compat.v1.flags.DEFINE_string("model_dir", "", "Directory to save model checkpoints")

tf.compat.v1.flags.DEFINE_string(
    "advtrain_attack_method", "MadryEtAl", "FastGradientMethod, MadryEtAl"
)
tf.compat.v1.flags.DEFINE_string(
    "attack_method", "MadryEtAl", "FastGradientMethod, MadryEtAl"
)
tf.compat.v1.flags.DEFINE_float("eps", 0.05, "maximal eps for attacks")
tf.compat.v1.flags.DEFINE_integer(
    "baseline_epoch", 1, "epoch of the checkpoint to load"
)
tf.compat.v1.flags.DEFINE_integer("batch_size", 64, "")
tf.compat.v1.flags.DEFINE_float("param", 0.01, "params for non-iterative attacks")
tf.compat.v1.flags.DEFINE_string("dataset", "cifar10", "mnist or cifar10 or cifar100")
...
def log_det(y_true, y_pred, num_model=FLAGS.num_models):
    bool_R_y_true = tf.not_equal(
        tf.ones_like(y_true) - y_true, zero
    )  # batch_size X (num_class X num_models), 2-D
    mask_non_y_pred = tf.boolean_mask(
        y_pred, bool_R_y_true
    )  # batch_size X (num_class-1) X num_models, 1-D
    mask_non_y_pred = tf.reshape(
        mask_non_y_pred, [-1, num_model, num_classes - 1]
    )  # batch_size X num_model X (num_class-1), 3-D
    mask_non_y_pred = mask_non_y_pred / tf.norm(
        mask_non_y_pred, axis=2, keepdims=True
    )  # batch_size X num_model X (num_class-1), 3-D
    matrix = tf.matmul(
        mask_non_y_pred, tf.transpose(mask_non_y_pred, perm=[0, 2, 1])
    )  # batch_size X num_model X num_model, 3-D
    all_log_det = tf.linalg.logdet(
        matrix + det_offset * tf.expand_dims(tf.eye(num_model), 0)
    )  # batch_size X 1, 1-D
    return all_log_det
...
def acc_metric(y_true, y_pred, num_model=FLAGS.num_models):
    ...
    return tf.keras.metrics.categorical_accuracy(y_t[0], ens_p)
...
def Loss_withEE_DPP(y_true, y_pred, num_model=FLAGS.num_models):
    ...
    CE_all = 0
    for i in range(num_model):
        CE_all += tf.keras.losses.categorical_crossentropy(y_t[i], y_p[i])
...
def model_eval_targetacc(
    ...
    if X_test is None or Y_test_target is None or Y_test is None:
        raise ValueError(
            "X_test argument and Y_test argument and Y_test_target argument"
            "must be supplied."
        )
    ...
    with sess.as_default():
        ...
        Y_cur_target = np.zeros(
            (args.batch_size,) + Y_test_target.shape[1:], dtype=Y_test_target.dtype
        )
...
def get_ensemble_diversity_values(
    sess,
    x,
    y,
    predictions,
    ...
    with sess.as_default():
        ...
        for batch in range(nb_batches):
            ...
            if feed is not None:
                feed_dict.update(feed)
            ensemble_diversity_records_batch = get_batch_ensemble_diversity.eval(
                feed_dict=feed_dict
            )

            ensemble_diversity_records = np.concatenate(
                (ensemble_diversity_records, ensemble_diversity_records_batch), axis=0
            )
